...
-(id)initWithAd:(id)currentAd{
    if(self = [super initWithAd:currentAd]){
        ...
        self.nativeContent = [self newsDictFromNativeContent:utf8PubContent];
    }
    return self;
}
...
-(void)openWithView:(UIView*)view{
   [ad attachToView:view];
    NSString* url = [self.nativeContent valueForKey:@"landingURL"];
    if ([self.nativeContent valueForKey:@"isAd"]) {
...
-(UIImage*)getImage{
    NSDictionary* iconDict = [self.nativeContent valueForKey:@"icon"];
