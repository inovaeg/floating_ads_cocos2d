...
@interface BubbleAdsManager(){

    NSMutableArray* inMobiAds;
//    AdBubbleScene* bubbleScene;
    BubbleAdWorld* bubbleAdWorld;
}
+ (id)sharedManager;
@end

@implementation BubbleAdsManager
    ...
    if (adImageUrl) {
        dispatch_async(dispatch_get_global_queue(DISPATCH_QUEUE_PRIORITY_DEFAULT, 0), ^(void) {
            ...
            dispatch_async(dispatch_get_main_queue(), ^{
                [bubbleAdWorld createBubbleWithAd:ad];
            });
        });
    }
}
...
+ (id)sharedManager {
    static BubbleAdsManager *sharedMyManager = nil;
